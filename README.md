...
- Random word selection from a list, or input your own word for someone else to guess.
- ASCII art to display hangman progress.
- Tracks missed and correct guesses.
- Clean, modular codebase split into multiple files.
- Choose from four difficulties: Easy, Medium, Hard, and Super Hard. Each difficulty changes the number of hints and excluded letters available.
- Excluded Letters: At the start of each game, several random letters that are not in the word are shown. You cannot guess these letters.
